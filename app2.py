...
def load_config() -> AppConfig:
    """Loads default configuration for 2-step gap dataset file path and remote release download URL."""
    return AppConfig(
        gaps_file=DEFAULT_GAPS2_FILE,
...
def ensure_dataset_exists(config: AppConfig) -> None:
    ...
    gaps2_path = config.gaps_file

    if os.path.exists(gaps2_path) and os.path.getsize(gaps2_path) > 100_000:
        return

    st.info(f"📦 2-Step Gap Database (`{gaps2_path}`) not found locally. Fetching remote storage (~2048 MB)...")
    ...
    try:
        ...
        st.error(
            ...
            f"Please run `cargo run --release --bin build_gaps -- 2` to generate `{gaps2_path}` locally.\n\n"
            f"Error details: {e}"
        )
        st.stop()
