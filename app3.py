...
DEFAULT_GAPS3_FILE = "gaps3.parquet"
DEFAULT_RELEASE_URL = "https://github.com/JunghunLeePhD/jumpchamp/releases/download/v1.0.0/gaps3.parquet"


def load_config() -> AppConfig:
    """Loads default configuration for 3-step gap dataset file path and remote release download URL."""
    return AppConfig(
        gaps_file=DEFAULT_GAPS3_FILE,
...
def ensure_dataset_exists(config: AppConfig) -> None:
    """Validates existence of gaps3.parquet; downloads from release asset if missing/corrupt."""
    gaps3_path = config.gaps_file

    if os.path.exists(gaps3_path) and os.path.getsize(gaps3_path) > 100_000:
        return

    st.info(f"📦 3-Step Gap Database (`{gaps3_path}`) not found locally. Fetching remote storage (~2048 MB)...")
    ...
    try:
        urllib.request.urlretrieve(config.release_url, gaps3_path, reporthook=_download_callback)
        progress_bar.empty()
        status_text.empty()
        ...
        progress_bar.empty()
        status_text.empty()
        if os.path.exists(gaps3_path):
            os.remove(gaps3_path)
        st.error(
            f"❌ Database (`{gaps3_path}`) not found locally and could not be fetched from remote storage.\n\n"
            f"Please run `cargo run --release --bin build_gaps -- 3` to generate `{gaps3_path}` locally.\n\n"
            f"Error details: {e}"
        )
        st.stop()
...
def query_prime_gaps(
    ...
    """Queries 3-step gap (k=3) frequency distribution by prime index range [min_idx, max_idx]."""
...
def render_math_definitions() -> None:
    """Renders LaTeX mathematical explanations for 3-step prime gap size."""
    with st.expander("📖 Mathematical Definitions & Notation", expanded=False):
        st.markdown(
            ...
            * **3-Step Gap ($\Delta_3$):** The arithmetic difference between primes separated by two intervening primes:
              $$\Delta_3(n) = p_{n+3} - p_n$$
...
def render_gap_distribution_chart(df: pd.DataFrame) -> None:
    ...
    fig = px.bar(
        ...
        y="frequency",
        text="percentage",
        labels={"diff_label": "3-Step Gap Size (Δ₃)", "frequency": "Frequency"},
...
def render_data_table(df: pd.DataFrame) -> None:
    ...
    display_df.columns = ['Rank', '3-Step Gap Size (Δ₃)', 'Frequency', 'Percentage']
...
def main():
    st.set_page_config(page_title="3-Step Prime Gap Explorer", page_icon="🦀", layout="wide")
